...
function display_pagenavigator(category, page, num_pages) {
    ...
    nav.querySelectorAll("#page-last").forEach((elem) => {
        if (page == num_pages) {
            ...
            elem.setAttribute(
                "href",
                `./notes.html?category=${category}&page=${num_pages}`
            );
        }
    });
    ...
    nav.querySelectorAll("#page-prev").forEach((elem) => {
        if (page == 1) {
            ...
            elem.setAttribute(
                "href",
                `./notes.html?category=${category}&page=${page - 1}`
            );
        }
    });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const num_notes = Array.from(
        document.querySelector("notes").children
    ).length;

    Array.from(document.querySelector("categories").children).forEach(
        (elem) => {
            if (elem.id === category) {
